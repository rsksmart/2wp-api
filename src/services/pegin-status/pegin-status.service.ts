...
import {BtcAddressUtils, calculateBtcTxHashSegWitAndNonSegwit, getTxSentAmountByAddress} from '../../utils/btc-utils';
      ...
      .then(async (btcTx: BitcoinTx) => {
        ...
        if (!this.canBeAPeginSender(btcTx)) {
          ...
          if (!this.isSentToFederationAddress(federationAddress, btcTx.vout)) {
            ...
            btcStatus.amountTransferred = this.fromSatoshiToBtc(getTxSentAmountByAddress(
              federationAddress,
              btcTx.vout
            ));
            if ( btcStatus.amountTransferred === 0) {
              const errorMessage = `Can not get set amount for address: ${federationAddress} in tx: ${btcTx.txId}`;
