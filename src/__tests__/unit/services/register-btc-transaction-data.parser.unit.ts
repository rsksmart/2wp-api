...
const encodeRegisterBtcTransactionParameters = (
  tx: string,
  height: number,
  pmt: string,
) => {
  ...
  return web3.eth.abi.encodeParameters(registerBtcTransactionAbi.inputs, [
    tx,
    height,
    pmt,
  ]);
...
const getFakeRegisterBtcTransactionData = () => {
  ...
  return remove0x(
    encodeRegisterBtcTransactionParameters(rawTx, 1, ensure0x('')),
  );
};
...
describe('Service: RegisterBtcTransactionDataParser', () => {
  ...
  it('parses a transaction with a PEGIN_BTC log properly', async () => {
    ...
    const tx = new RskTransaction();
    tx.data =
      REGISTER_BTC_TRANSACTION_SIGNATURE + getFakeRegisterBtcTransactionData();
    const log = new Log();
  ...
  it('parses a transaction with a LOCK_BTC log properly', async () => {
    ...
    expect(result).to.be.instanceOf(PeginStatusDataModel);
    if (result) {
      expect(result.rskRecipient).to.be.Undefined(); // ATM the parsing of a LOCK_BTC can't set the recipient
      expect(result.status).to.be.equal(PeginStatus.LOCKED);
      expect(result.btcTxId).to.be.equal(txHash);
    }
  });
  ...
  it('parses a transaction with REJECTED_PEGIN and RELEASE_REQUESTED logs as a rejected pegin with refund', async () => {
    ...
    expect(result).to.be.instanceOf(PeginStatusDataModel);
    if (result) {
      expect(result.rskRecipient).to.be.Undefined(); // ATM the parsing of a REJECTED_PEGIN can't set the recipient
